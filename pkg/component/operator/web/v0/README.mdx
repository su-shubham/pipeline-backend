...
| Include Markdown | `include-markdown`  | boolean | Indicate whether to include the markdown content associated with this page in the 'markdown' field |
...
Recipe for the [Web crawler](https://instill.tech/instill-ai/pipelines/web-crawler/playground) pipeline.
...
component:
  crawler:
    type: web
    task: TASK_CRAWL_WEBSITE
    input:
      target-url: ${variable.url}
      allowed-domains:
...
variable:
    url:
      title: url
      description: The url of the web you want to crawl
      instill-format: string
output:
  link:
    title: Link
    value: ${crawler.output.pages[page_number].markdown}
```
